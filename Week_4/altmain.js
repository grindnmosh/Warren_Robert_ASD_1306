...
//The functions below can go inside or outside the pageinit function for the page in which it is needed.
		 ...
		 $.couch.db("qaexp2").view("app/real", {
			success		:	function(data) {
				...
				$.each(data.rows, function(index, info) {
					...
					var editBut = $("<button data-key='"+id+"'><a href='#add'> Edit QA</a></button>");
					editBut.on('click', function(){
						editKey = $(this).data('id');
						editItem(editKey);
					});
					var deleteBut = $("<button data-key='"+id+"'><a href='#addQa' id='delete"+id+"'>Delete QA</a></button>");
					deleteBut.on('click', function(){
						editKey = $(this).data('id');
						deleteItem(editKey);
					});	
					
	    			makeSubList.append(couched).append(editBut).append('<br>').append(deleteBut).append('<hr />').appendTo("#qaContent");
			 	});
	...
	var editItem = function(editKey) {
		$.couch.db("qaexp2").openDoc(editKey, {
